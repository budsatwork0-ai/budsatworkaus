        ...
        <div className="grid gap-3 overflow-x-auto" style={{ gridTemplateColumns: `repeat(${visibleStages.length}, minmax(200px, 1fr))` }}>
          {visibleStages.map((stage) => {
            const stageApplicants = filtered.filter((a) => {
              if (group === 'community' && stage === 'verify') return !['intake'].includes(a.stage);
                  ...
                  {stageApplicants.map((a) => {
                    ...
                    return (
                      <div key={a.id} className={`${glass} p-3`}>
                        ...
                        {a.email && <p className="text-[10px] truncate mt-0.5" style={{ color: brand.muted }}>{a.email}</p>}
                        ...
                        {community && a.stage === 'intake' && (
                          <button onClick={() => updateStage(a.id, 'verify')} className="mt-2 w-full text-[11px] px-2 py-1 rounded-lg font-medium text-white" style={{ background: '#4338CA' }}>
                            Mark contacted
                          </button>
                        )}
                        {community && (
                          <a href={`mailto:${a.email}`} className="mt-1 block w-full text-[11px] px-2 py-1 rounded-lg font-medium text-center border" style={{ borderColor: '#E5E7EB', color: brand.muted }}>
                            Email →
                          </a>
                        )}
                        {!community && nextStage && (
                          <button onClick={() => updateStage(a.id, nextStage)} className="mt-2 w-full text-[11px] px-2 py-1 rounded-lg font-medium text-white" style={{ background: brand.primary }}>
                            Move to {STAGE_LABELS[nextStage]}
                          </button>
                        )}
                        {!community && a.stage === 'ready' && !a.user_id && (
                          <button onClick={() => activateApplicant(a)} disabled={activating === a.id} className="mt-1 w-full text-[11px] px-2 py-1 rounded-lg font-medium border disabled:opacity-50" style={{ background: '#ECFDF5', color: '#065F46', borderColor: '#A7F3D0' }}>
                ...
                return (
                  <tr key={a.id} className="border-b border-slate-50 hover:bg-slate-50">
                    <td className="px-4 py-3">
                      ...
                      {community && a.quality_business_name && <p className="text-[11px] text-slate-400">{a.quality_business_name}</p>}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        {community && a.stage === 'intake' && (
                          <button onClick={() => updateStage(a.id, 'verify')} className="text-xs px-2 py-1 rounded-lg text-white" style={{ background: '#4338CA' }}>Mark contacted</button>
                        )}
                        {community && (
                          <a href={`mailto:${a.email}`} className="text-xs px-2 py-1 rounded-lg border" style={{ borderColor: '#E5E7EB', color: brand.muted }}>Email</a>
                        )}
                        {!community && nextStage && (
                          <button onClick={() => updateStage(a.id, nextStage)} className="text-xs px-2 py-1 rounded-lg text-white" style={{ background: brand.primary }}>Advance</button>
                        )}
                        {!community && a.stage === 'ready' && !a.user_id && (
                          <button onClick={() => activateApplicant(a)} disabled={activating === a.id} className="text-xs px-2 py-1 rounded-lg font-medium border disabled:opacity-50" style={{ background: '#ECFDF5', color: '#065F46', borderColor: '#A7F3D0' }}>
                            {activating === a.id ? 'Sending...' : 'Convert to Staff'}
                          </button>
                        )}
                        {!community && a.user_id && <span className="text-[11px] text-slate-400">Staff active</span>}
                      </div>
                    </td>
                  </tr>
                );
