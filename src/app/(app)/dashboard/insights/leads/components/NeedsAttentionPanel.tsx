    ...
    >
      ...
      <div className="mb-4 grid grid-cols-2 gap-2 xl:grid-cols-3">
        {(Object.entries(counts) as Array<[AttentionItem['category'], number]>).map(([cat, n]) => {
          ...
          const t = dashboardTheme.night!.tempTone[meta.tone.toUpperCase() as 'HOT' | 'WARM' | 'COLD' | 'LOST'];
          const isActive = n > 0;
          return (
            <div
              key={cat}
              ...
              style={{
                ...
                borderColor: isActive ? t.ring : dashboardTheme.night!.color.border,
            ...
            >
              ...
              <p className="mt-1 text-xl font-semibold tabular-nums" style={{ color: isActive ? t.text : dashboardTheme.night!.color.muted }}>
                {n}
              </p>
            </div>
          );
      ...
      {items.length > limit ? (
        <p className="mt-3 text-center text-[11px]" style={{ color: dashboardTheme.night!.color.subtle }}>
...
function AttentionRow({ item }: { item: AttentionItem }) {
  ...
  return (
    <Link
      ...
      style={{ borderColor: dashboardTheme.night!.color.border, background: 'rgba(255,255,255,0.012)' }}
    >
      <span
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="truncate text-sm font-semibold" style={{ color: dashboardTheme.night!.color.text }}>{item.title}</span>
          <NightChip tone={meta.tone}>
            {meta.label}
          </NightChip>
        </div>
        <div className="mt-1 flex items-center gap-2 text-[11px]" style={{ color: dashboardTheme.night!.color.muted }}>
      ...
      <div className="flex flex-shrink-0 items-center gap-3 text-right">
        <div>
          ...
          <div className="mt-0.5 flex items-center justify-end gap-1.5 text-[10px] uppercase tracking-[0.14em]" style={{ color: dashboardTheme.night!.color.subtle }}>
