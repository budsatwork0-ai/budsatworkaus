...
import type { AgentLifecycleState } from '@/lib/agents/agents/foreman';
...
interface Props {
  initial: {
    lobbyState: ForemanLobbyState | null;
    agents: AgentRow[];
    runs: RunRow[];
    pending: ActionRow[];
    insights: InsightRow[];
...
export function ForemanConsole({ initial }: Props) {
  const [lobbyState, setLobbyState]       = useState<ForemanLobbyState | null>(initial.lobbyState);
  ...
  const [foremanRunning, setForemanRunning] = useState(false);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'foreman_lobby_states' },
        (p) => {
          if (p.eventType === 'INSERT' || p.eventType === 'UPDATE') {
            const s = p.new as ForemanLobbyState & { is_current: boolean };
            if (s.is_current) {
              setLobbyState(s);
              pushEvent('foreman', 'briefing', s.summary ?? 'Foreman briefing updated');
            }
          }
        })
      .subscribe();
  ...
  // Derive live agent states: merge foreman snapshot with real-time running state
  ...
  async function handleRunForeman() {
    setForemanRunning(true);
    try {
      await fetch('/api/agents/foreman', { method: 'POST' });
    } finally {
      setForemanRunning(false);
    }
  }
      ...
      <header className="sticky top-0 z-30 border-b border-zinc-800 bg-zinc-950/90 backdrop-blur-sm">
        <div className="max-w-[1600px] mx-auto px-5 py-2.5 flex items-center gap-4 min-w-0">
          ...
          <p className="text-xs text-zinc-400 leading-snug line-clamp-1 min-w-0 flex-1">
            {lobbyState?.summary ?? 'Waiting for Foreman briefing…'}
          </p>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={handleRunForeman}
              disabled={foremanRunning}
              className="px-3 py-1.5 text-xs rounded-md bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 disabled:opacity-50 transition-colors"
            >
              {foremanRunning ? 'Briefing…' : 'Run Foreman'}
            </button>
        ...
        <main className="col-span-12 lg:col-span-9 space-y-4">
          ...
          {!lobbyState && (
            <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-8 text-center">
              <p className="text-zinc-400 text-sm mb-3">No briefing available. Run The Foreman to generate the operational view.</p>
              <button
                onClick={handleRunForeman}
                disabled={foremanRunning}
                className="px-4 py-2 text-sm rounded-md bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 disabled:opacity-50"
              >
                {foremanRunning ? 'Running Foreman…' : 'Run Foreman now'}
              </button>
            </div>
          )}
