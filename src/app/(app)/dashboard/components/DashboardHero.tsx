...
function pct(n: number): string {
  return `${Math.round(Math.abs(n) * 10) / 10}%`;
}
...
export function DashboardHero({ metrics, moneyFlow, recentActivity }: {
  ...
  const peakIdx = trend.length ? trend.reduce((b, t, i, a) => (t.revenue > a[b].revenue ? i : b), 0) : -1;
  const services = [...(metrics.revenueByService ?? [])].sort((a, b) => b.amount - a.amount).slice(0, 5);
  const customers = moneyFlow.incoming?.revenueByCustomer ?? [];
  const customerCount = customers.length;
  const customerAvatars = customers.slice(0, 5);
      ...
      <div className="space-y-5">
        ...
        <section className={CARD}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <KpiTile highlighted label="Customers" value={customerCount.toLocaleString()} note="billed this month" />
            <KpiTile label="Revenue · MTD" value={formatCurrency(metrics.goals.currentRevenue)} delta={metrics.goals.revenueChange} />
          </div>
