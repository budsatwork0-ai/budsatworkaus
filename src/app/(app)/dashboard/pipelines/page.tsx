...
function Skeleton() {
  return (
    <div className={`${glass} rounded-xl p-3 animate-pulse`} style={{ borderColor: dashboardTheme.color.border, background: dashboardTheme.color.card }}>
...
export default function PipelinesPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: dashboardTheme.color.bg }}>
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-bold" style={{ color: dashboardTheme.color.primary }}>Workflows</h1>
      ...
      <div className="grid gap-3 grid-cols-2 md:grid-cols-4 mb-6">
        {[
          { label: 'Total jobs', value: isLoading ? '—' : String(orders.filter(o => o.status !== 'cancelled').length), color: dashboardTheme.color.text },
          { label: 'Active', value: isLoading ? '—' : String(activeJobs), color: dashboardTheme.color.primary },
          { label: 'Completed today', value: isLoading ? '—' : String(completedToday), color: '#22c55e' },
          { label: 'Stages', value: String(STAGES.length), color: dashboardTheme.color.muted },
        ].map(s => (
            ...
            return (
              <div key={stage.key} className="flex-1 min-w-[180px]">
                <div className="flex items-center gap-2 mb-2.5 px-1">
                  ...
                  <div className="text-xs font-semibold" style={{ color: dashboardTheme.color.text }}>{stage.label}</div>
                ...
                <div className="space-y-2">
                  {isLoading
                    ...
                    : stageOrders.length === 0
                      ? (
                        <div className="rounded-xl border border-dashed p-4 text-center" style={{ borderColor: dashboardTheme.color.border }}>
                          <div className="text-xs" style={{ color: dashboardTheme.color.muted }}>No jobs</div>
                        </div>
                      )
                      : stageOrders.map(order => (
                        <div
                          ...
                          style={{ borderColor: dashboardTheme.color.border, background: dashboardTheme.color.card, borderLeftWidth: 3, borderLeftColor: stage.color }}
                        >
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-[10px] font-medium" style={{ color: dashboardTheme.color.muted }}>{shortId(order.id)}</span>
                            <span className="text-[10px]" style={{ color: dashboardTheme.color.muted }}>
                              {order.scheduled_time || order.scheduled_date || '—'}
                            </span>
                          </div>
                          <div className="text-sm font-medium" style={{ color: dashboardTheme.color.text }}>{order.customer_name}</div>
